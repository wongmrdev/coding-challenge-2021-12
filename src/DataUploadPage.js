...
function App() {
  const { searchString, handleSequenceChange, setSearchString } =
  ...
  async function fetchDnaSequences(searchString = '', limit = 500) {
    try {
      const dnaSequenceData = await API.graphql(
        graphqlOperation(listDnaSequences, {
  ...
  function handleTextAreaSubmit() {
    ...
    if (hasDnaSequenceProperties(dnaSequence)) {
      addDnaSequence(dnaSequence).then((response) => {
        setUploadStatusList([
          ...uploadStatusList,
          `manual input: ${response.dnaSequence.message}, ${response.creator.message}`,
        ]);
      });
    } else {
      setUploadStatusList([
        ...uploadStatusList,
        'manual input: check your input',
      ]);
    }
  ...
  function hasDnaSequenceProperties(item) {
    if (
      ...
      console.error(
        'item is missing a property or property data-type is incorrect'
      );
      return false;
    }
  }
  ...
  async function handleFileListSubmit(fileList) {
    //FileList is an HTML Collection
    setIsDisabled(true);
    setLoading(true);
    let newItems = [];
  ...
  function handleFileListSubmitResponse(fileName, addDnaSequenceResposne) {
    let dnaS = addDnaSequenceResposne.dnaSequence.status
      ? addDnaSequenceResposne.dnaSequence.status
      : null;
    let creatorS = addDnaSequenceResposne.creator.status
      ? addDnaSequenceResposne.creator.status
      : null;
    let dnaM = addDnaSequenceResposne.dnaSequence.message
      ? addDnaSequenceResposne.dnaSequence.message
      : null;
    let creatorM = addDnaSequenceResposne.creator.message
      ? addDnaSequenceResposne.creator.message
      : null;
  ...
  async function addDnaSequence(dnaSequenceObject) {
    ...
    let response = {};
    try {
      if (!hasDnaSequenceProperties(dnaSequenceObject)) {
        return false;
      }
      const dnaSequence = await API.graphql(
        graphqlOperation(getDnaSequence, { id: dnaSequenceObject.id })
      );
      const creator = await API.graphql(
        graphqlOperation(getCreator, { id: dnaSequenceObject.creator.id })
      );
      if (dnaSequence.data.getDnaSequence) {
        ...
        await API.graphql(
          graphqlOperation(createDnaSequence, { input: dnaSequenceObjectInput })
        );
        response = {
          ...response,
          dnaSequence: { status: 'success', message: 'Upload success' },
        };
      }
      if (creator.data.getCreator) {
        response = {
          ...response,
          creator: { status: 'failed', message: 'creator Already Exists' },
        };
      } else {
        await API.graphql(
          graphqlOperation(createCreator, { input: creatorObjectInput })
        );
    ...
    <div>
      <div className="App">
        ...
        <header className="App-header">
          <label id="data-upload-title" htmlFor="createDnaSequence">
            Add JSON format DNA Sequence
          </label>
        </header>
      </div>
      ...
      <div className="vertical-flex-container">
        <input
          ...
          multiple
          accept=".json, .txt"
          onChange={(event) => handleFileListSubmit(event.target.files)}
        ></input>
        {loading ? (
          <p>loading</p>
        ) : (
          <ul>
            {uploadStatusList.map((status, index) => (
              <li key={index}>
                {index === 0 ? '' : index + '.'} {status}
              </li>
            ))}
          </ul>
        )}
